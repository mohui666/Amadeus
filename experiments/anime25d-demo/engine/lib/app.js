...
async function loadModel(source,name){
  ...
  try{
    ...
    if(ticket!==loadTicket)return;
    const {psd,pre,rig}=await parseModel(buf,undefined,ticket);
    if(ticket!==loadTicket)return;
    applyRig(rig);
...
function fit(){
  const st=document.getElementById('stage'), s=Math.min(st.clientWidth/CW, st.clientHeight/(CH*demoCrop));
...
function animate(now,dt){
  ...
  if(auto.blink&&!camLive&&!activePreset){
    ...
    if(blinkT>=0){ blinkT+=dt;
      const d=blinkT, hold=blinkVariant===2?0.08:0.04; let v;
  ...
  for(const k in cur){tgt[k]=RT.clamp(tgt[k],...parameterRanges[k]);cur[k]+=(tgt[k]-cur[k])*(1-Math.exp(-dt*(k.startsWith('eyeOpen')?60:14)));}
  ...
  if(irisBounceT>=0){
    ...
    const scale=1+0.015*Math.sin(p*Math.PI*4.0)*damp;
    const squash=0.01*Math.sin(p*Math.PI*4.0+Math.PI/2)*damp;
  ...
  e.breath=auto.idle?0.5+0.5*Math.sin(t*2*Math.PI/5.6):0;
  e.breathHead=auto.idle?0.5+0.5*Math.sin(t*2*Math.PI/5.6-0.6):0;   // head follows chest with a lag
...
function render(e){
  ...
  for(const {L} of active){if(L.bn!=='eyewhite'&&L.name!=='mouth_open_1')continue;const bit=L.name==='mouth_open_1'?4:L.side==='L'?1:2;
  ...
  for(const {L,alpha} of active){
    ...
    if((L.bn==='irides'&&L.side)||L.name==='mouth_open_2'||L.name==='mouth_open_3'){const bit=L.bn==='mouth_open'?4:L.side==='L'?1:2;gl.enable(gl.STENCIL_TEST);gl.stencilFunc(gl.EQUAL,bit,bit);gl.stencilOp(gl.KEEP,gl.KEEP,gl.KEEP);}
