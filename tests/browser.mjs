...
async function configureApi(page) {
  ...
  await page.getByLabel('API Key', { exact: true }).fill('fixture-chat-secret');
  await page.getByRole('button', { name: 'OpenAI 账号登录', exact: true }).click();
  await expect(page.getByLabel('模型（留空使用账号默认）')).toHaveValue('');
  await page.getByRole('button', { name: 'API 接入', exact: true }).click();
  await expect(page.getByLabel('模型名称', { exact: true })).toHaveValue('fixture-vision');
  await expect(page.getByLabel('API 地址', { exact: true })).toHaveValue('http://127.0.0.1:11434/v1');
  assert.deepEqual(await page.getByRole('combobox', { name: /^接口协议/ }).locator('option').allTextContents(), ['OpenAI 兼容', 'Responses', 'Anthropic']);
  await page.getByLabel('API 地址', { exact: true }).fill('http://127.0.0.1:11434/v1');
  await page.getByLabel('模型名称', { exact: true }).fill('fixture-vision');
  await page.getByLabel('API Key', { exact: true }).fill('fixture-chat-secret');
  ...
  await page.getByRole('button', { name: '语音 API', exact: true }).click();
  await page.getByRole('combobox', { name: /^语音识别/ }).selectOption('browser');
  await page.getByLabel('语音 API Key', { exact: true }).fill('fixture-voice-secret');
  ...
  assert.match(stored, /fixture-chat-secret/);
  assert.match(stored, /fixture-voice-secret/);
}
...
try {
  ...
  assert.equal(persisted.chat.apiKey, 'fixture-chat-secret');
  assert.equal(persisted.tts.apiKey, 'fixture-voice-secret');
  await page.getByRole('button', { name: '打开设置', exact: true }).click();
  await expect(page.getByLabel('API Key', { exact: true })).toHaveValue('fixture-chat-secret');
  await page.getByRole('button', { name: '关闭设置', exact: true }).click();
  record({ check: 'API/account switch, saved keys, fixture image+SSE+TTS and refresh history', status: 'PASS' });
  ...
  await bilingualPage.getByRole('button', { name: '系统语音', exact: true }).click();
  await bilingualPage.getByRole('button', { name: '使用本机配置', exact: true }).click();
  await expect(bilingualPage.getByRole('combobox', { name: '语音 API 格式', exact: true })).toHaveValue('gpt-sovits');
  ...
  await bilingualPage.getByRole('button', { name: '完成', exact: true }).click();
  ...
  for (const width of [320, 393, 430]) {
    ...
    await expect(native.page.getByRole('button', { name: '完成', exact: true })).toBeInViewport();
  }
  ...
  await native.page.getByRole('button', { name: '语音 API', exact: true }).click();
  await native.page.getByRole('tab', { name: '角色', exact: true }).click();
  await native.page.getByRole('combobox', { name: /^她如何回复/ }).selectOption('zh');
  await native.page.getByRole('button', { name: '完成', exact: true }).click();
  ...
  await expect(smart.page.locator('.memory-entry:visible')).toHaveCount(1);
  await expect(smart.page.getByText('已更新的历史 · 1', { exact: true })).toBeVisible();
  await expect(smart.page.locator('.memory-entry').first()).toContainText('用户现居上海。');
  await smart.page.locator('.memory-entry:visible summary').first().click();
  ...
  const savedVoice = await session({ width: 360, height: 800 }, { native: true });
  const voicePage = savedVoice.page;
  await voicePage.getByRole('button', { name: '打开设置', exact: true }).click();
  await voicePage.getByRole('tab', { name: '声音', exact: true }).click();
  await expect(voicePage.getByLabel('语音 API 地址', { exact: true })).toHaveCount(0);
  await expect(voicePage.getByText('高级声音设置', { exact: true })).toBeVisible();
  await voicePage.screenshot({ path: resolve(outputDir, 'voice-settings-simple.png'), animations: 'disabled' });
  await voicePage.getByText('高级声音设置', { exact: true }).click();
  await voicePage.getByRole('button', { name: '语音 API', exact: true }).click();
  await expect(voicePage.getByRole('combobox', { name: '语音 API 格式', exact: true })).toHaveValue('openai');
  assert.deepEqual(await voicePage.getByRole('combobox', { name: '语音 API 格式', exact: true }).locator('option').allTextContents(), ['OpenAI 兼容', 'GPT-SoVITS', 'ElevenLabs']);
  await voicePage.getByLabel('语音 API 地址', { exact: true }).fill('https://voice.fixture.test/v1');
  await voicePage.getByLabel('语音 API Key', { exact: true }).fill('saved-native-voice-key');
  await voicePage.getByLabel('语音模型', { exact: true }).fill('custom-voice-model');
  await voicePage.getByRole('combobox', { name: '语音 API 格式', exact: true }).selectOption('elevenlabs');
  await expect(voicePage.getByLabel('语音 API Key', { exact: true })).toHaveValue('');
  await voicePage.getByRole('combobox', { name: '语音 API 格式', exact: true }).selectOption('openai');
  await expect(voicePage.getByLabel('语音 API Key', { exact: true })).toHaveValue('saved-native-voice-key');
  await voicePage.getByRole('button', { name: '系统语音', exact: true }).click();
  await voicePage.getByRole('button', { name: '语音 API', exact: true }).click();
  await expect(voicePage.getByLabel('语音 API 地址', { exact: true })).toHaveValue('https://voice.fixture.test/v1');
  // Closing with X and reloading must save without pressing Done.
  await voicePage.getByRole('button', { name: '关闭设置', exact: true }).click();
  await voicePage.reload({ waitUntil: 'networkidle' });
  await voicePage.getByRole('button', { name: '打开设置', exact: true }).click();
  await voicePage.getByRole('tab', { name: '声音', exact: true }).click();
  await voicePage.getByText('高级声音设置', { exact: true }).click();
  await expect(voicePage.getByLabel('语音 API Key', { exact: true })).toHaveValue('saved-native-voice-key');
  await expect(voicePage.getByLabel('语音模型', { exact: true })).toHaveValue('custom-voice-model');
  assert.ok(await voicePage.evaluate(() => localStorage.getItem('native.settings').includes('saved-native-voice-key')));
  await voicePage.getByRole('combobox', { name: '语音 API 格式', exact: true }).scrollIntoViewIfNeeded();
  await voicePage.screenshot({ path: resolve(outputDir, 'voice-settings-api.png'), animations: 'disabled' });
  await noOverflow(voicePage);
  assert.deepEqual(savedVoice.errors, []);
  await savedVoice.context.close();
  record({ check: 'simple voice settings, three API formats, protocol preservation and automatic Android key persistence after X/reload', status: 'PASS' });
